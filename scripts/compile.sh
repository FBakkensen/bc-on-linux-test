...
ALL_PROJECTS=(app test integration-test)
...
for proj in "${requested[@]}"; do
    case "$proj" in
        app) ;;
        test|integration-test) needs_app=1 ;;
        *) echo "compile.sh: unknown project '$proj' (expected: app, test, integration-test)" >&2; exit 2 ;;
    esac
done
