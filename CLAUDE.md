...
- **`BC_KEEP_APP_IDS` workaround**: this workspace sets `BC_KEEP_APP_IDS` in `bc-linux/.env` because the upstream entrypoint clears the Microsoft test framework apps on first boot but does not reinstall them unless that var is non-empty. Do not blank it — first boot will publish a test app against missing framework dependencies and fail.
